...
export default   {
  getAllCategory:function({commit}){
    ...
    axios.get(`${API_BASE}/categorys`).then(response=>{
      commit(ALL_CATEGORYS_SUCCESS,response.data)
    })
  },
  changeStatusCategory:function({commit},payload){
    axios.post(`${API_BASE}/category/change_Status`,{
      _id:payload
    }).then(respone=>{
  ...
  deleteCategory:function({commit},payload){
